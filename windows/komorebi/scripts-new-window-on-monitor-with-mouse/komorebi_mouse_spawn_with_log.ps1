...
function Get-MonitorIndexAtPoint {
    ...
    if ($monitors.Count -eq 0) {
        Write-Diagnostic `
            ...
            -Data ([ordered]@{ operation_id = $OperationId; cursor_x = $X; cursor_y = $Y })
...
function Invoke-KomorebicCommand {
    ...
    if (-not $isStateCommand -or $exitCode -ne 0) {
        $commandData["output"] = Limit-DiagnosticText -Text $outputText -MaximumLength 8192
    }
...
function Get-KomorebiState {
    ...
    $result = Invoke-KomorebicCommand -Arguments @("state") -OperationId $OperationId
...
function Request-ForegroundWindow {
    ...
    $target = [IntPtr]$Hwnd
    $initialForegroundHwnd = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetForegroundWindow().ToInt64()
    $isWindow = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::IsWindow($target)
    ...
    if ($initialForegroundHwnd -eq $Hwnd) {
        Write-Diagnostic `
            ...
            -Data ([ordered]@{ operation_id = $OperationId; hwnd = $Hwnd })
        return $true
    }

    $wasIconic = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::IsIconic($target)
    $showWindowResult = $null
    if ($wasIconic) {
        # SW_RESTORE
        $showWindowResult = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::ShowWindowAsync(
            $target,
            9
        )
    }

    $currentThread = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetCurrentThreadId()
    $foregroundWindow = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetForegroundWindow()
    $foregroundThread = 0
    if ($foregroundWindow -ne [IntPtr]::Zero) {
        $foregroundThread = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetWindowThreadProcessId(
            $foregroundWindow,
            [IntPtr]::Zero
        )
    }

    $targetThread = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetWindowThreadProcessId(
        $target,
        [IntPtr]::Zero
    )
    ...
    try {
        if ($foregroundThread -ne 0 -and $foregroundThread -ne $currentThread) {
            $attachedForeground = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::AttachThreadInput(
                $currentThread,
                $foregroundThread,
                $true
            )
        ...
        if (
            ...
            $attachedTarget = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::AttachThreadInput(
                $currentThread,
                $targetThread,
                $true
            )
        ...
        $bringWindowToTopResult = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::BringWindowToTop(
            $target
        )
        $setForegroundWindowResult = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::SetForegroundWindow(
            $target
        )
    }
    finally {
    ...
    $finalForegroundHwnd = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetForegroundWindow().ToInt64()
...
function Wait-ForKomorebiWindowFocus {
    ...
    do {
        ...
        $lastForegroundHwnd = [KomorebiMouseSpawnerDiagnosticsV1.NativeMethods]::GetForegroundWindow().ToInt64()
        $osFocused = ($lastForegroundHwnd -eq $Hwnd)
        $lastKomorebiFocused = Test-KomorebiWindowFocused -State $state -Hwnd $Hwnd

        $locationData = $null
        $locations = Get-WindowLocationMap -State $state
        $locationKey = [string]$Hwnd
        if ($locations.ContainsKey($locationKey)) {
            $locationData = Get-WindowLocationDiagnosticData -Location $locations[$locationKey]
        }
    ...
    try {
        ...
        for ($attempt = 1; $attempt -le 2; $attempt++) {
            ...
            $moveResult = Invoke-KomorebicCommand `
                -Arguments @("move-to-monitor", [string]$TargetMonitorIndex) `
                -OperationId $OperationId
            ...
            do {
                $verificationPoll++
                $verifyState = Get-KomorebiState -OperationId $OperationId
                ...
                if ($verifyLocations.ContainsKey($locationKey)) {
                    ...
                    $verifyLocationData = Get-WindowLocationDiagnosticData -Location $verifyLocation
        ...
        if ($restoreOriginMonocle) {
            $focusOrigin = Invoke-KomorebicCommand `
                -Arguments @("focus-monitor", [string]$originMonitorIndex) `
                -OperationId $OperationId
            ...
            [void](Invoke-KomorebicCommand `
                -Arguments @("focus-monitor", [string]$TargetMonitorIndex) `
                -OperationId $OperationId)
        }
...
try {
    ...
    Write-Diagnostic `
        ...
        -Data ([ordered]@{ path = $script:KomorebicPath })

    $versionResult = Invoke-KomorebicCommand -Arguments @("--version")
    if ($versionResult.ExitCode -eq 0) {
        Write-Diagnostic `
            -Level "Info" `
            -Event "komorebic.version" `
            -Data ([ordered]@{ version_output = $versionResult.Output.Trim() })
    }
    else {
        Write-Diagnostic `
            -Level "Warn" `
    ...
    $ownsMutex = $true
    Write-Diagnostic -Level "Debug" -Event "mutex.acquired"
    ...
    $pump.Start()
    Write-Diagnostic -Level "Debug" -Event "pipe.reader_started"
    ...
    Write-Diagnostic `
        ...
        -Data ([ordered]@{ pipe_name = $pipeName })

    if (-not $pump.Connected.Wait(5000)) {
        Write-Diagnostic `
            -Level "Error" `
            -Event "pipe.connection_timeout" `
            -Data ([ordered]@{ pipe_name = $pipeName; timeout_ms = 5000 })
    ...
    $monitorTopology = @()
    $baselineMonitors = @(Get-RingElements -Ring $baselineNotification.state.monitors)
    for ($monitorIndex = 0; $monitorIndex -lt $baselineMonitors.Count; $monitorIndex++) {
        $monitor = $baselineMonitors[$monitorIndex]
        $rectangle = Get-KomorebiMonitorRectangle -Monitor $monitor
        $workspaceCount = @(Get-RingElements -Ring $monitor.workspaces).Count
        $monitorName = $null
        $monitorDevice = $null
    ...
    while ($true) {
        ...
        if (-not $pump.Messages.TryTake([ref]$message, 1000)) {
            ...
            Write-Diagnostic `
                ...
                -Data ([ordered]@{ queue_depth = $pump.Messages.Count })
            continue
        }
        ...
        try {
            ...
            $eventWindow = Get-NotificationEventWindow -Notification $notification
            $eventHwnd = $null
            ...
            $stateFocusedMonitorProperty = $notification.state.monitors.PSObject.Properties["focused"]
            if ($null -ne $stateFocusedMonitorProperty -and $null -ne $stateFocusedMonitorProperty.Value) {
                $stateFocusedMonitorIndex = [int]$stateFocusedMonitorProperty.Value
            }

            $stateMouseFollowsFocus = $null
            $stateMouseFollowsFocusProperty = $notification.state.PSObject.Properties["mouse_follows_focus"]
            if ($null -ne $stateMouseFollowsFocusProperty) {
                $stateMouseFollowsFocus = $stateMouseFollowsFocusProperty.Value
            }

            $stateFocusFollowsMouse = $null
            $stateFocusFollowsMouseProperty = $notification.state.PSObject.Properties["focus_follows_mouse"]
            if ($null -ne $stateFocusFollowsMouseProperty) {
                $stateFocusFollowsMouse = $stateFocusFollowsMouseProperty.Value
            }
            ...
            $windowData = Get-WindowDiagnosticData -Window $eventWindow
            ...
            if ($null -ne $eventHwnd -and $queueLagMs -ge 500) {
                Write-Diagnostic `
                    -Level "Warn" `
    ...
    $fatalError = $true
    Write-ExceptionDiagnostic -Event "process.fatal" -ErrorRecord $_
    throw
}
finally {
    Write-Diagnostic `
    ...
    if ($subscribed) {
        try {
            $unsubscribeResult = Invoke-KomorebicCommand `
                -Arguments @("unsubscribe-pipe", $pipeName)
    ...
    if ($null -ne $pump) {
        try {
            $pump.Dispose()
            Write-Diagnostic -Level "Debug" -Event "pipe.reader_disposed"
        }
        catch {
            Write-ExceptionDiagnostic `
    ...
    elseif ($null -ne $pipe) {
        try {
            $pipe.Dispose()
            Write-Diagnostic -Level "Debug" -Event "pipe.disposed"
        }
        catch {
            Write-ExceptionDiagnostic `
    ...
    if ($ownsMutex -and $null -ne $mutex) {
        try {
            $mutex.ReleaseMutex()
            Write-Diagnostic -Level "Debug" -Event "mutex.released"
        }
        catch {
            Write-ExceptionDiagnostic `
